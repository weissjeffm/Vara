...
(defn prime? "Does every prime, up to the square root of mynum, leave 
a remainder when you divide mynum by it? Then mynum is prime"
  [mynum]
  (every? #(> (rem mynum %) 0) 
...
(defn rotations [n]
  (let [len  (count (str n))]
    (map #(Long/parseLong %)
	 (rest (take len (iterate left-circular n))))))
  ...
  (reduce 
   (fn [hasha hashb] 
     (if (> (:value hasha)
       ...
       hasha
       hashb)) 
   (map (fn [x] {:index x :value (count (mysequence x))})
